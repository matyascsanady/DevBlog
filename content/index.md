---
title: Welcome
---

Greetings traveller!
You somehow stumble upon my personal blog page.

I'm Mátyás Csanády currently working as a BIM expert / Revit API developer for a Hungarian AEC company. I completed my bachelors in Civil engineering, where i was focusing on BIM. After that I had a masters in CIT (Computer Information Technology). Now these two papers I got are mostly meaningless, as they gave me very little to no knowledge about my current work. Currently around half of time at work is spent on BIM support (mostly practical in Revit) and the other time I develop custom Revit add-ins for the company.

Next to my work, I currently teach at BME in Budapest at both the Architecture and Civil Engineering faculty. For the architects I teach Revit software use, a beginner course, to teach them the basics of Revit modelling. For the Civil Engineering faculty, as a guest lecturer, I talk about the different automation options available in Revit and showcase some stuff we did as a company.

If you are interested in my carrier in more detail checkout my [[Life]].

I'm happy that you are here, be sure to check out some of my stuff.
- Revit API facts and *"funny things"* -> like [[1 - Type parameters available from instances]]
- Other Dev related topics on this blog
- My [GitHub](https://github.com/matyascsanady) contains some interesting repositories (at least for me) for example:
	- The [public archive](https://github.com/matyascsanady/RevitEnhance-Deprecated) of a pyRevit extension for my bachelors.
	- The [public repository](https://github.com/matyascsanady/Benchmarking-Revit-Automation-Workflows) of my masters.